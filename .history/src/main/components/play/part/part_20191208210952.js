...
class Part extends Component {
  ...
  constructor(x, y, partData, checkIfOnTarget) {
    super();
    ...
    this.checkIfOnTarget = checkIfOnTarget;
  ...
  drop() {
    this.view.animatePropertyTo("size", Part.dropPercent, 50, {
      onComplete: () => {
        ...
        this.checkIfOnTarget(this.partData.target);
      }
    });
  }
  ...
  checkIfOnTarget() {
    ...
    if (
      Utils.distance(
        this.targetOffset.x + this.partData.target.x,
        this.targetOffset.y + this.partData.target.y,
        this.view.x,
        this.view.y
