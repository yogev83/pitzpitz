...
class PuzzleStructure extends Structure {
  ...
  parsePuzzleImage(puzzleImage) {
    ...
    $.each(puzzleImage.parts.wrongs, (i, wrong) => {
      ...
      wrongPartBuffer = new Part(locationBuffer.x, locationBuffer.y, wrong);
  ...
  createLocations(count, puzzle) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      attempts = 0;
      while (
        !this.isFarEnough(locationBuffer, locations, puzzle) &&
  ...
  getRandomLocation(puzzleData) {
    ...
    let x = Utils.getRandomIntInclusive(
      PuzzleStructure.bounds.MIN_X,
      PuzzleStructure.bounds.MAX_X
    );
    let y = Utils.getRandomIntInclusive(
      PuzzleStructure.bounds.MIN_Y,
      PuzzleStructure.bounds.MAX_Y
  ...
  isFarEnough(testedLocation, locations, puzzle) {
    let farEnough = true;
