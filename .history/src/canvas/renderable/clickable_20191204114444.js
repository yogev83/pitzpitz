...
class Clickable extends Frameable {
  ...
  hitTest(ctx, x, y) {
    ...
    if (typeof this.options.hitTest == "function") {
      inside = this.options.hitTest(ctx, x, y) && this.colorHitTest(ctx, x, y);
    } else {
      inside =
        this.x < x &&
  ...
  colorHitTest(ctx, x, y) {
    ctx.getImageData(this.x, this.y, this.options.width, this.options.height);
